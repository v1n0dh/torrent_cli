...
long long int bencode_decode_int(const std::string &torr_str, int &cnt);
...
long long int bencode_decode_int(const std::string &torr_str, int &cnt)
{
	...
	long long int value = std::stoll(c_num);
	cnt += c_num.length() + 2;
	return value;
}
...
Json::Value bencode_decode(const std::string &torr_str, int &cnt)
{
	if (torr_str[0] == BENCODE_BEGIN_INT) return Json::Value(static_cast<Json::UInt64>(bencode_decode_int(torr_str, cnt)));
